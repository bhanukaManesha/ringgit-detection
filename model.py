...
class YOLOModel :
    ...
    def __init__(self):
        ...
        self.metrics = YOLOMetrics()

        now = datetime.now()
        folder = 'models/{:%Y%m%d-%H%M%S}'.format(now)
        os.makedirs(folder)
        self.history_checkpoint = YOLOMetrics.HistoryCheckpoint(folder=folder)

        self.model_checkpoint = ModelCheckpoint('{}/model_weights.h5'.format(folder), save_weights_only=True)
